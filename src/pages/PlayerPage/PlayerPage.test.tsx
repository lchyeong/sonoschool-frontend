...
const testStreamResponse: ProtectedLectureStream = {
  ...
  playbackWatermarkText: '테스트회원1234',
...
describe('PlayerPage', () => {
  ...
  it('opens speed and quality setting panels independently', async () => {
    ...
    expect(await screen.findByRole('dialog', { name: '재생 속도 설정 패널' })).toBeInTheDocument();
    ...
    fireEvent.click(screen.getByRole('button', { name: '화질 설정' }));

    expect(screen.queryByRole('dialog', { name: '재생 속도 설정 패널' })).not.toBeInTheDocument();
  ...
  it('renders resource lessons as a downloadable attachment table', async () => {
    ...
    expect(screen.getByText('957KB')).toBeInTheDocument();
  ...
  it('shows an unsupported browser notice when HLS playback is unavailable', async () => {
    ...
    expect(
      await screen.findByText('현재 브라우저에서는 재생이 지원되지 않습니다.'),
    ).toBeInTheDocument();
  ...
  it('renders and submits the problem lecture as a lesson item', async () => {
    ...
    expect(screen.queryByText('문항수 1문항')).not.toBeInTheDocument();
    ...
    expect(screen.getByLabelText('채점 결과 요약')).toHaveTextContent('0/ 100');
    expect(screen.getByLabelText('채점 결과 요약')).toHaveTextContent('재도전 필요');
    expect(screen.getByLabelText('채점 결과 요약')).toHaveTextContent('목표 점수는 70점입니다.');
    expect(screen.getByLabelText('문항 리뷰')).toHaveTextContent('오답');
    expect(screen.getByLabelText('문항 리뷰')).toHaveTextContent('선택 답안');
  ...
  it('shows the latest graded result and allows reviewing questions after reload', async () => {
    ...
    expect(await screen.findByText('채점 결과')).toBeInTheDocument();
    expect(screen.getByLabelText('채점 결과 요약')).toHaveTextContent('100/ 100');
    ...
    expect(screen.queryByRole('button', { name: '다시 풀기' })).not.toBeInTheDocument();
    expect(screen.getByLabelText('문항 리뷰')).toHaveTextContent('정답');
    expect(screen.getByLabelText('문항 리뷰')).toHaveTextContent('선택 답안');
    expect(screen.getByText('정답 해설입니다.')).toBeInTheDocument();
    expect(screen.queryByText('선택 답안: 정답')).not.toBeInTheDocument();
    expect(screen.queryByText(/^정답:/)).not.toBeInTheDocument();
