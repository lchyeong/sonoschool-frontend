...
const ProgramsPage = () => {
  ...
  } = useProgramCatalogActions(data?.featuredLectures ?? []);
  ...
  return (
    <div className={styles['container']}>
      ...
      {/* 대표 과정 섹션은 전체 과정 허브에서도 바로 프로그램 상세로 들어갈 수 있게 도와줍니다. */}
      <section className={styles['section']}>
        <div className={styles['sectionHeader']}>
          <p className={styles['sectionEyebrow']}>대표 과정</p>
          <h2 className={styles['sectionTitle']}>대표 과정 먼저 보기</h2>
          <p className={styles['sectionDescription']}>
            소노스쿨의 전체 커리큘럼 중 문의가 잦거나 흐름을 이해하기 좋은 대표 과정을 먼저
            배치했습니다.
          </p>
        </div>
      ...
      <section className={styles['bottomSection']}>
        ...
        <article className={styles['brandNoteCard']}>
          ...
          <p className={styles['brandNoteDescription']}>
            인프런처럼 여러 강사가 각자 다른 포맷으로 과정을 올리는 구조가 아니라, 장은희 강사가
