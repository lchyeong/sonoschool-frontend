...
export const useProgramCatalogActions = (lectures: readonly ProgramLectureCard[]) => {
  ...
  const enrollmentsQuery = useMyEnrollmentsQuery(isAuthenticated && programIds.length > 0);
  ...
  const enrolledProgramIds = useMemo(() => {
    return new Set(
      (enrollmentsQuery.data ?? [])
        .filter((enrollment) => blocksProgramCartAction(enrollment))
  ...
  const handleAddToCart = (lecture: ProgramLectureCard) => {
    ...
    if (enrolledProgramIds.has(payload.programId)) {
      showToast({
